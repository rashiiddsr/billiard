...
## Rekomendasi Logika Tombol Hardware (Mode Darurat)

Jika ada tombol hardware manual untuk situasi urgent, desain paling aman adalah menjadikannya **override terbatas** terhadap relay listrik, tetapi **bukan override terhadap data billing**.

### Prinsip utama

1. **Pisahkan dua state:**
   - `PowerState` (fisik lampu/meja ON/OFF, dari relay)
   - `BillingState` (sesi tagihan ACTIVE/CLOSED, dari server)
2. Tombol hardware hanya boleh mengubah `PowerState`.
3. `BillingState` hanya boleh berubah lewat API billing (start/stop/checkout) agar perhitungan biaya tetap konsisten.

### Aturan prioritas yang disarankan

Gunakan prioritas event seperti berikut:

- **Priority 1: Emergency OFF** (tombol darurat fisik)
  - Relay langsung OFF.
  - Device mengirim event ke server: `EMERGENCY_OFF` + timestamp + alasan.
  - Jika ada sesi billing aktif, sesi tetap aktif, tetapi diberi flag `interrupted_by_emergency=true` untuk audit.
- **Priority 2: Sistem billing normal**
  - Billing kirim command `LIGHT_ON/LIGHT_OFF` sesuai status sesi.
- **Priority 3: Manual non-darurat** (jika ada mode service)
  - Hanya boleh saat tidak ada sesi aktif atau role tertentu.

### Supaya tidak bentrok dengan sistem billing

- **Rule keras:** saat ada billing aktif di meja tertentu, tombol hardware untuk meja itu wajib ditolak oleh API (`accepted=false, blockedReason=ACTIVE_BILLING`) sehingga firmware harus mengembalikan relay ke state billing.
- Jika meja dinyalakan manual dari tombol (tanpa billing), backend menandai meja `OCCUPIED` agar start billing ditolak dengan pesan yang jelas ke kasir (toast warning).
- Kondisi `OCCUPIED` tanpa sesi billing aktif juga harus memblokir fitur kontrol sistem lain (contoh: testing owner/developer) karena sumber ON berasal dari hardware, jadi sistem hanya boleh memberi pesan untuk mematikan tombol hardware terlebih dulu.
- Tambahkan status lock sementara, contoh: `hardwareOverrideUntil` (misal 30–120 detik) setelah tombol darurat ditekan.
- Selama lock aktif:
  - command dari billing ke relay ditahan/antri,
  - UI menampilkan badge **Emergency Override Active**.
- Setelah lock selesai, server melakukan **state reconciliation**:
  - jika sesi masih aktif -> kirim `LIGHT_ON` lagi,
  - jika sesi selesai -> pertahankan `LIGHT_OFF`.

### Checklist implementasi minimal

- Log semua event hardware ke audit trail (`who/what/when/device/table`).
- Device wajib idempotent (event tombol sama tidak diproses berulang).
- Tambahkan cooldown debounce pada firmware (contoh 300–500ms) agar satu tekan tidak terbaca berkali-kali.
- Dashboard menampilkan perbedaan `PowerState` vs `BillingState` agar operator cepat tahu ada mismatch.

Dengan pola ini, tombol hardware tetap menjadi jalur keselamatan saat kejadian luar biasa, tapi akurasi billing tidak rusak karena sumber kebenaran transaksi tetap di server.


### Integrasi firmware yang disarankan (ringkas)

Agar profesional dan minim flicker, ubah flow firmware menjadi **request dulu ke API, baru ubah relay lokal**:

1. Tombol ditekan -> kirim `POST /iot/devices/button` (`action: TOGGLE|ON|OFF`, `relayChannel`, optional `tableId`).
2. Jika response `accepted=true`, firmware set relay sesuai `shouldOn`.
3. Jika `accepted=false`, firmware **jangan toggle** (atau segera rollback), lalu ikuti `shouldOn` dari server.

Contoh response block saat billing aktif:

```json
{
  "accepted": false,
  "blockedReason": "ACTIVE_BILLING",
  "shouldOn": true,
  "message": "Billing sedang aktif, tombol hardware dinonaktifkan untuk meja ini."
}
```

Dengan pola ini, tombol tidak bisa mengganggu sesi billing berjalan, dan billing juga tidak bisa dimulai saat meja sedang ON manual.
