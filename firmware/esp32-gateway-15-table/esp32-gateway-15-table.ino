...
// =========================
// Mapping meja -> relay channel ESP + button channel MCP
// =========================
struct TableConfig {
  const char* tableId;
  uint8_t relayChannel;   // 0..15 (index ke RELAY_GPIO_PINS)
  uint8_t buttonChannel;  // 0..15 pin MCP23X17
};
...
void printMappingTable() {
  Serial.println("=== Relay(GPIO ESP32) / Button(MCP23X17) -> Table ID Mapping ===");
  for (int i = 0; i < TABLE_COUNT; i++) {
    ...
    Serial.printf("MejaIndex=%d | tableId=%s | relayCH=%d | relayGPIO=%d | buttonMCP=%d\n",
                  i + 1,
                  TABLES[i].tableId,
                  relayCh,
                  relayGpio,
...
// =========================
// Button handling (MCP23X17 input)
// =========================
void initMcpButtons() {
  Wire.begin();
  ...
  Serial.println("[MCP] button input siap");
}
...
void scanButtons() {
  ...
  for (int i = 0; i < TABLE_COUNT; i++) {
    const uint8_t btnCh = TABLES[i].buttonChannel;
    bool current = mcp.digitalRead(btnCh);
    ...
    if ((now - btnLastChangeMs[i]) > 40 && btnStable[i] != btnLastRead[i]) {
      ...
      if (oldStable == HIGH && btnStable[i] == LOW) {
        ...
        Serial.printf("[Button MCP] table=%s toggled -> %s\n", TABLES[i].tableId, lampState[i] ? "ON" : "OFF");
      }
    }
  }
}
