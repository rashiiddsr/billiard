...
import { IsString, IsOptional, IsBoolean, IsArray, ArrayMinSize, ArrayMaxSize, IsInt, Min, Max, IsIn } from 'class-validator';
...
export class IotController {
  ...
  async handleButtonEvent(
    ...
    @Headers('x-nonce') nonce: string,
    @Headers('x-signature') signature: string,
    @Body() body: ButtonEventDto,
  ) {
    const rawBody = JSON.stringify(body);
    return this.iotService.handleButtonEvent(deviceId, token, timestamp, nonce, signature, rawBody, body);
  }
