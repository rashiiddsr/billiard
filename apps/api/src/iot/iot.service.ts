...
export class IotService {
  ...
  async handleButtonEvent(
    ...
    signature: string,
    body: string,
    payload: { relayChannel: number; tableId?: string; action: ButtonAction },
  ) {
    ...
    const table = await this.prisma.table.findFirst({
      where: {
        ...
        relayChannel: payload.relayChannel,
        isActive: true,
      },
      select: {
        id: true,
    ...
    if (!table) {
      throw new NotFoundException('Table mapping untuk relay channel tidak ditemukan');
    }

    if (payload.tableId && payload.tableId !== table.id) {
      return {
        accepted: false,
        blockedReason: 'TABLE_MISMATCH',
        tableId: table.id,
        tableName: table.name,
        relayChannel: payload.relayChannel,
        shouldOn: table.status !== TableStatus.AVAILABLE,
        message: 'tableId dari hardware tidak sesuai mapping relay di server.',
      };
    }
    ...
    if (table.status === TableStatus.MAINTENANCE) {
      return {
        accepted: false,
        blockedReason: 'TABLE_MAINTENANCE',
        tableId: table.id,
        tableName: table.name,
        relayChannel: payload.relayChannel,
        shouldOn: true,
        message: 'Meja sedang maintenance/testing, tombol hardware dinonaktifkan.',
      };
    }

    const action = payload.action || 'TOGGLE';
    const targetOn = action === 'ON' ? true : action === 'OFF' ? false : table.status === TableStatus.AVAILABLE;

    await this.prisma.table.update({
      where: { id: table.id },
      data: { status: targetOn ? TableStatus.OCCUPIED : TableStatus.AVAILABLE },
    });
    ...
    return {
      ...
      message: targetOn
        ? 'Meja dihidupkan manual via tombol hardware. Billing sementara dikunci.'
        : 'Meja dimatikan manual via tombol hardware.',
    };
  }
  ...
  async sendCommand(
    ...
    const pendingStatuses = [IoTCommandStatus.PENDING, IoTCommandStatus.SENT];
    const cutoff = new Date(Date.now() - dedupeWindowSeconds * 1000);
    const recentCommands = await this.prisma.iotCommand.findMany({
      where: {
    ...
    const existing = recentCommands.find((cmd) => {
