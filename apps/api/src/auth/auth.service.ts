...
export class AuthService {
  ...
  async reAuth(userId: string, credential: string, type: 'password' | 'pin' = 'pin') {
    ...
    const token = this.jwtService.sign(
      ...
      { secret: this.config.get('JWT_SECRET') || resolveEnvValue('JWT_SECRET'), expiresIn: '5m' },
    );
  ...
  async refreshTokens(refreshToken: string) {
    try {
      const payload = this.jwtService.verify(refreshToken, {
        secret: this.config.get('JWT_REFRESH_SECRET') || resolveEnvValue('JWT_REFRESH_SECRET'),
      });
  ...
  private async generateTokens(userId: string, email: string, role: string) {
    ...
    const accessToken = this.jwtService.sign(payload, {
      ...
      expiresIn: this.config.get('JWT_ACCESS_EXPIRES_IN') || resolveEnvValue('JWT_ACCESS_EXPIRES_IN') || '15m',
    });

    const refreshToken = this.jwtService.sign(payload, {
      secret: this.config.get('JWT_REFRESH_SECRET') || resolveEnvValue('JWT_REFRESH_SECRET'),
      expiresIn: this.config.get('JWT_REFRESH_EXPIRES_IN') || resolveEnvValue('JWT_REFRESH_EXPIRES_IN') || '7d',
    });
