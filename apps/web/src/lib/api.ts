...
// Session panjang: access token 8 jam, refresh token 30 hari
...
// Mutex: cegah multiple refresh sekaligus
...
// Response interceptor — hanya redirect login kalau refresh token juga sudah benar-benar tidak valid
  ...
  async (error: AxiosError) => {
    ...
    if (status === 401 && !original?._retry && !isRefreshRequest) {
      ...
      try {
        ...
        // Refresh token juga expired → baru logout
        clearAuthStorage();
        redirectToLogin();
      }
    }
