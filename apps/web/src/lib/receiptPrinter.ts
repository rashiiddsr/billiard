...
async function sendToDesktopPrintBridge(payload: { title: string; text?: string; html?: string; logoUrl?: string | null }) {
...
export async function printReceiptText(receiptText: string, title = 'Struk', options?: { logoUrl?: string | null }) {
  ...
  const resolvedLogoUrl = resolveImageUrl(options?.logoUrl);

  const printedByQz = await printViaQzTray({ title, text: receiptText, logoUrl: resolvedLogoUrl }).catch(() => false);
  if (printedByQz) return true;

  const printedByBridge = await sendToDesktopPrintBridge({ title, text: receiptText, logoUrl: resolvedLogoUrl });
  if (printedByBridge) return true;

  const logoMarkup = resolvedLogoUrl
    ? `<img src="${escapeHtml(resolvedLogoUrl)}" alt="logo" style="display:block;margin:0 auto 8px;max-width:72px;max-height:72px;object-fit:contain;" />`
    : '';
...
export function wrapAndCenterReceiptText(text: string, width = 32) {
  const cleanText = text.replace(/\s+/g, ' ').trim();
  if (!cleanText) return [];
  ...
  for (const word of words) {
    ...
    lines.push(centerReceiptText(currentLine, width));
    currentLine = word;
  }

  if (currentLine) lines.push(centerReceiptText(currentLine, width));
  return lines;
}
