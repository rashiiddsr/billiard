...
const ACCESS_TOKEN_REFRESH_BUFFER_MINUTES = 5;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const shouldRefreshSession = useCallback(() => {
    ...
    const refreshAfterMs = (60 - ACCESS_TOKEN_REFRESH_BUFFER_MINUTES) * 60 * 1000;
  ...
  useEffect(() => {
    ...
    const syncSessionOnVisible = async () => {
      if (inFlight || !shouldRefreshSession()) return;
      inFlight = true;
      try {
        await refreshAuthSession();
      } catch {
        // tidak force logout saat gangguan jaringan; retry otomatis terjadi saat request 401
      } finally {
        inFlight = false;
      }
    };
