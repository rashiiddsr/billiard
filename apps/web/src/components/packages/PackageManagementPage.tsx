...
export default function PackageManagementPage() {
  ...
  const submit = async () => {
    ...
    if (form.fnbItems.some((x) => !x.quantity || x.quantity < 1)) {
      return toast.error('Qty item F&B minimal 1');
    }
    if (form.fnbItems.some((x) => x.unitPrice === undefined || x.unitPrice < 0)) {
      return toast.error('Harga item F&B tidak valid');
    }
                ...
                {form.fnbItems.map((row, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2">
                    ...
                    <div className="col-span-6 md:col-span-2">
                      ...
                      <input type="number" min={1} className="input" value={row.quantity ?? ''} onChange={(e) => updateFnbRow(idx, { quantity: e.target.value === '' ? undefined : Number(e.target.value) })} />
                    </div>
                    <div className="col-span-6 md:col-span-3">
                      <label className="label">Harga Item (Rp) <span className="text-red-500">*</span></label>
                      <input type="number" min={0} className="input" value={row.unitPrice ?? ''} onChange={(e) => updateFnbRow(idx, { unitPrice: e.target.value === '' ? undefined : Number(e.target.value) })} />
                    </div>
