...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen overflow-x-hidden bg-gradient-to-br from-sky-50 via-blue-50 to-violet-100 text-slate-800">
      <Sidebar collapsed={collapsed} mobileOpen={mobileOpen} onClose={() => setMobileOpen(false)} />
      <div className={`flex min-h-screen min-w-0 flex-1 flex-col transition-all ${collapsed ? 'md:ml-24' : 'md:ml-72'}`}>
        ...
        <main className="flex-1 min-w-0 overflow-auto px-2 py-4 md:px-6">{children}</main>
      </div>
    </div>
  );
}
