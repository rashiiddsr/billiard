  ...
  return (
    <div className="p-6 space-y-6">
          ...
          >
            ...
            {devices.map((d) => (
              <option key={d.id} value={d.id}>
                {d.id} {d.name ? `(${d.name})` : ''} {d.isOnline ? '• online' : '• offline'}
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="card">
        ...
        <p className="text-sm text-slate-400 mb-3">Model baru hanya memakai 1 device gateway aktif. Device lain boleh tetap tersimpan tapi tidak dipakai untuk command relay.</p>
        <div className="space-y-2">
          {devices.map((d) => (
            <div key={d.id} className="flex justify-between items-center p-3 bg-slate-700 rounded-lg text-sm">
              <div>
                ...
                <p className="text-slate-300">Dipakai sebagai gateway: {settings?.gatewayDeviceId === d.id ? 'Ya' : 'Tidak'}</p>
              </div>
