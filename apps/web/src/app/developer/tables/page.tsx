...
export default function DeveloperTablesPage() {
  ...
  const [form, setForm] = useState<any>({ name: '', iotDeviceId: '', gpioPin: '', hourlyRate: '' });
  ...
  const openCreate = () => {
    ...
    setEditing(null);
    setForm({ name: '', iotDeviceId: devices[0].id, gpioPin: GPIO_OPTIONS[0], hourlyRate: '' });
    setShowForm(true);
  };
  ...
  const save = async () => {
    if (!form.name || !form.iotDeviceId || form.gpioPin === '' || form.hourlyRate === '') {
      toast.error('Nama, device IoT, GPIO, dan harga wajib diisi');
      return;
    }
      ...
      <div className="card p-0 overflow-hidden">
        <div className="table-wrapper">
          <table className="data-table">
            <thead>
              <tr>
                <th>ID</th><th>Nama</th><th>Device</th><th>GPIO</th><th>Harga/Jam</th><th>Status</th><th>Aksi</th>
              </tr>
            </thead>
            <tbody>
              {loading ? <tr><td colSpan={7} className="text-center py-6">Memuat...</td></tr> : tables.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-6 text-slate-500">Belum ada meja</td></tr>
              ) : tables.map((t) => (
                <tr key={t.id}>
