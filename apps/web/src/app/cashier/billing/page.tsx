              ...
              {session && (
                <div className="mb-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                  {session.rateType === 'OWNER_LOCK' ? (
                    <>
                      <p className="text-sm font-semibold text-indigo-700">Sesi Owner Aktif</p>
                      <p className="mt-1 text-xs text-slate-500">Tanpa countdown & tanpa tagihan transaksi.</p>
                    </>
                  ) : (
                    <>
                      {remaining && <p className={`font-mono text-sm font-bold ${remaining.isWarning ? 'animate-pulse text-red-500' : 'text-emerald-600'}`}>{remaining.text}</p>}
              ...
              <div className="grid grid-cols-4 gap-2">
                {!isOccupied ? (
                  ...
                  <button
                    ...
                    className="btn-danger col-span-4 py-2 text-sm"
                  >
                    Hentikan Sesi
                  </button>
                ) : session.rateType === 'OWNER_LOCK' ? (
                  <>
                    <a href={`/cashier/orders?sessionId=${session.id}`} className="btn-primary col-span-2 py-2 text-center text-sm">
                      Pesan F&B Owner
                    </a>
                    <button onClick={() => openDetailModal(session)} className="btn-secondary col-span-2 py-2 text-sm">
                      Lihat Detail Owner
                    </button>
                  </>
                ) : (
                  <>
                    ...
                    <a
                      ...
                      className={`btn-primary py-2 text-center text-sm ${(session.payments || []).length > 0 ? 'pointer-events-none opacity-40' : ''}`}
                    >
                      Tambah F&B
                    </a>
            ...
            <table className="data-table bg-white">
              ...
              <tbody>
                {activeSessions.map((s) => {
                  const remaining = s.rateType === 'OWNER_LOCK' ? null : getRemainingTime(s.endTime);
                  return (
                    <tr key={s.id}>
                      ...
                      <td className="text-xs text-slate-400">{s.rateType === 'OWNER_LOCK' ? '-' : new Date(s.endTime).toLocaleTimeString('id-ID')}</td>
                      <td>
                        {s.rateType === 'OWNER_LOCK' ? <span className="text-xs text-indigo-600">Tanpa Countdown</span> : remaining && <span className={`font-mono text-sm ${remaining.isWarning ? 'font-bold text-red-500' : 'text-emerald-600'}`}>{remaining.text}</span>}
                      </td>
                      <td className="font-medium">{s.rateType === 'OWNER_LOCK' ? 'Rp 0' : formatCurrency(s.totalAmount)}</td>
                      ...
                      <td>
                        <div className="flex flex-wrap gap-2">
                          {isOwner ? (
                            ...
                            <>
                              <a href={`/cashier/orders?sessionId=${s.id}`} className="rounded-lg bg-blue-100 px-2 py-1 text-xs text-blue-700 hover:bg-blue-200">Pesan F&B Owner</a>
                              <button onClick={() => openDetailModal(s)} className="rounded-lg bg-indigo-100 px-2 py-1 text-xs text-indigo-700 hover:bg-indigo-200">Detail Owner</button>
                            </>
                          ) : (
                            <>
                              {(s.payments || []).length === 0 && (
                              ...
                              {(s.payments || []).length === 0 && <a href={`/cashier/orders?sessionId=${s.id}`} className="rounded-lg bg-blue-100 px-2 py-1 text-xs text-blue-700 hover:bg-blue-200">+F&B</a>}
                              <button onClick={() => openDetailModal(s)} className="rounded-lg bg-indigo-100 px-2 py-1 text-xs text-indigo-700 hover:bg-indigo-200">Detail</button>
                              <button
                                onClick={() => {
                                  setSelectedSession(s);
                                  setModal('stop');
          ...
          {isOwner ? (
            <div className="space-y-4">
              <div className="rounded-lg border border-indigo-200 bg-indigo-50 p-3 text-sm text-indigo-800">
                Sesi owner berjalan tanpa countdown dan tanpa tagihan. Sesi akan berakhir saat Anda klik <b>Hentikan Sesi</b>.
              </div>
              <div className="flex gap-2">
                <button onClick={() => setModal(null)} className="btn-secondary flex-1">Batal</button>
