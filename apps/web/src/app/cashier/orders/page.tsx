...
export default function OrdersPage() {
  ...
  const submitStandaloneCheckout = async () => {
    ...
    const expectedTotal = Math.max(cartTotal - discount, 0);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white p-5">
            ...
            <div className="mb-3 grid grid-cols-2 gap-2">
              <input type="number" className="input" placeholder="Diskon" value={discountAmount} onChange={(e) => setDiscountAmount(e.target.value)} />
              <input type="number" className="input" placeholder="Uang diterima" value={amountPaid} onChange={(e) => setAmountPaid(e.target.value)} />
            </div>
            <div className="mb-4 rounded bg-slate-50 p-2 text-sm font-semibold">Total: {formatCurrency(Math.max(cartTotal - (parseFloat(discountAmount || '0') || 0), 0))}</div>
