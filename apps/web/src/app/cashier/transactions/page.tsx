  ...
  const reprintReceipt = async () => {
    ...
    const receiptHtml = buildBusinessReceiptHtml({
      ...
      bodyRows: `
        ...
        ${(detail.billingSession?.amount || 0) > 0 ? `<div class="row"><span>Billiard</span><span>${formatCurrency(detail.billingSession?.amount || 0)}</span></div>` : ''}
        ${(detail.packageUsages || []).length > 0 ? '<div class="line"></div><div class="bold">Rincian Paket</div>' : ''}
        ${packageRows}
        <div class="line"></div>
        <div class="bold">F&B Tambahan</div>
        ${fnbExtraRows}
        <div class="line"></div>
        <div class="row bold"><span>TOTAL</span><span>${formatCurrency(detail.total || 0)}</span></div>
        <div class="row"><span>Diterima</span><span>${formatCurrency(detail.amountPaid || 0)}</span></div>
